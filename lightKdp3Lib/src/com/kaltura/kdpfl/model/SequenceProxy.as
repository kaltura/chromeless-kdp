package com.kaltura.kdpfl.model
{
	...
	public class SequenceProxy extends Proxy
	{
		...
		public function hasMediaLoaded () : Boolean
		{
			...
			if ( (mediaProxy.vo.entryUrl == activePlugin().entryUrl) || (activePlugin().mediaElement) )
			{
				return true;
			}
		...
		public function playNextInSequence () : void
		{
			...
			if ( hasMediaElement() )
			{
				...
				if ( !hasMediaLoaded() )
				{
					sendNotification(NotificationType.CHANGE_MEDIA, {entryUrl:activeSequenceArr[activeIndex].entryUrl});
				}
				else
				{
					if(activePlugin().entryUrl != "null")
					{
						vo.isAdLoaded = true;
					}
		 ...
		 * 
		 */		
		public function startMidSequence(sendAdOpportunity:Boolean, cuePointObj:Object = null):void 
		{
