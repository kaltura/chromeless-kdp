package tv.freewheel.wrapper.kaltura
{
	...
	public class FreeWheelMediator extends SequenceMultiMediator 
	{
		...
		override public function handleNotification(notification:INotification):void{
			...
			switch (notification.getName()){
				...
				case END_INTERACTIVE_AD:
					playerMediator.player.removeEventListener(MouseEvent.CLICK, onInteractiveClick);
					endSlot();
					break;
		...
		public function playSlotElement(slot:FWSlotElement):void{
			...
			if (!playerMediator.playerSprite.parent.contains(this.slotBase))
				playerMediator.playerSprite.parent.addChild(this.slotBase);
		...
		private function prepareSwfElement(slot:FWSlotElement):SWFElement {
			...
			playerMediator.player.addEventListener(MouseEvent.CLICK, onInteractiveClick);
			return swfElement;
		}
		...
		private function resetSlotBase():void{
			...
			if (playerMediator.playerSprite && !playerMediator.playerSprite.parent.contains(this.slotBase)){
				playerMediator.playerSprite.parent.addChild(this.slotBase);
			}
		...
		private function get player():MediaPlayer{
			if (!this._player && this.playerMediator){
				this._player = this.playerMediator.player;
			}
			return this._player;
		}
		...
		public function get playerDimension():Rectangle{
			if (this.player)
				return new Rectangle(playerMediator.playerSprite.x, playerMediator.playerSprite.y, playerMediator.playerSprite.width, playerMediator.playerSprite.height);
			else
